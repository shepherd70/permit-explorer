...
K.reset();
check('reset restores everything', digits(el('k-count').textContent)===String(N) && K.f.nbh==='' && K.f.cats.size===K.allCatNames.length && K.isFullRange());
console.log('\n--- global year range (drives both panes) ---');
const firstY=snapshot.bper.first.slice(0,4), lastY=String(Math.max(+snapshot.bper.last.slice(0,4), ...P.map(p=>+(p.issued||'0').slice(0,4))));
check('year axis spans BC Stats first year → newest permit year', K.years[0]===firstY && K.years[K.years.length-1]===lastY, K.years);
check('year selects populated and defaulted to the full range', el('f-y0').options.length===K.years.length && el('f-y0').value===firstY && el('f-y1').value===lastY);
check('scope badge shows the full range', el('yr-scope').textContent.includes(firstY+'–'+lastY), el('yr-scope').textContent);
const yr=[...new Set(P.map(p=>p.issued&&p.issued.slice(0,4)).filter(Boolean))].sort()[0];
K.setYears(yr,yr);
check('single year: permits count only that year', digits(el('k-count').textContent)===String(P.filter(p=>p.issued&&p.issued.startsWith(yr)).length), el('k-count').textContent);
const bperYr=snapshot.bper.series.total.filter(p=>p.m.startsWith(yr));
const bperYrSum=bperYr.reduce((a,p)=>a+(p.v||0),0);
check('single year: BC Stats value KPI recomputed for that year', el('b-total').textContent===fmt$(bperYrSum), [el('b-total').textContent, fmt$(bperYrSum)]);
check('single year: BC Stats KPI label and section title carry the year', el('b-total-l').textContent.includes(yr) && el('sec-a-title').textContent.includes(yr), [el('b-total-l').textContent, el('sec-a-title').textContent]);
check('single year: by-year chart has one bar, monthly chart only that year\'s months', charts['c-byear'].data.labels.length===1 && charts['c-byear'].data.labels[0]===yr && charts['c-bmonth'].data.labels.length===bperYr.length, [charts['c-byear'].data.labels, charts['c-bmonth'].data.labels.length]);
check('single year: permits section title carries the year', el('sec-b-title').textContent.includes(yr), el('sec-b-title').textContent);
K.setYears('2019','2021');
check('2019–2021: no permits, BC Stats only, explanatory insight', digits(el('k-count').textContent)==='0' && charts['c-byear'].data.labels.join(',')==='2019,2020,2021' && el('insights').innerHTML.includes('begin in December 2025'), [el('k-count').textContent, charts['c-byear'].data.labels]);
const s1921=snapshot.bper.series.total.filter(p=>p.m>='2019-01'&&p.m<='2021-12').reduce((a,p)=>a+(p.v||0),0);
check('2019–2021: value KPI sums exactly those months', el('b-total').textContent===fmt$(s1921), [el('b-total').textContent, fmt$(s1921)]);
check('2019–2021: peak year is 2019 with the LNG note', el('b-peak').textContent==='2019' && el('insights').innerHTML.includes('LNG Canada'), el('b-peak').textContent);
K.setYears(null,'2018');
check('moving "to" below "from" snaps the range to a single year', K.yearRange().join('-')==='2018-2018' && el('f-y0').value==='2018' && el('f-y1').value==='2018', K.yearRange());
K.setYears('2026',null);
check('moving "from" above "to" snaps the other end', K.yearRange().join('-')==='2026-2026', K.yearRange());
K.setYears(firstY,lastY);
check('selecting the axis ends restores the full range (nulls, no URL params)', K.isFullRange() && digits(el('k-count').textContent)===String(N) && el('b-total').textContent===fmt$(bperTotal));
